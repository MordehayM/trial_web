...
interface Example {
  id: string;
  maskedText: string;
  unpaintedText: string;
  targetText: string;
  maskedAudio: string;
  unpaintedAudio: string;
  targetAudio: string;
  maskedSpectrogram: string;
  unpaintedSpectrogram: string;
...
function AudioPlayer({ src }: { src: string }) {
  ...
  return (
    <div className="flex flex-col gap-4 items-start">
      ...
      <button
        className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white font-semibold rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-transform duration-200"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-gray-800">Unpainted Signal</h3>
          </div>
          <p className="text-sm text-gray-600 bg-gray-50 p-3 rounded">{example.unpaintedText}</p>
          <AudioPlayer src={example.unpaintedAudio} />
          <img 
            src={example.unpaintedSpectrogram} 
            alt="Unpainted Spectrogram" 
            className="w-full rounded-lg shadow-sm"
          />
        </div>
